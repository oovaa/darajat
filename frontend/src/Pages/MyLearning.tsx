...
interface Task {
  id: number;
  type: "video" | "quiz" | "reading" | "practice" | "somethingElse";
  title: string;
  progress: number;
  url: string;
}
    ...
    const fetchedLessons = [
      ...
      },
      {
        ...
        tasks: [
          ...
          {
            id: 2,
            title: "Video Lecture: Understanding Functions",
            type: "video",
            url: "#",
            progress: 0,
          },
      ...
      },
      {
        ...
        subject: "Math",
        tasks: [
          { id: 1, title: "Some quiz", type: "quiz", url: "#", progress: 100 },
        ],
      },
    ];
      ...
      <div className="week flex flex-col-reverse lg:grid lg:grid-cols-12 gap-2 lg:gap-10">
          ...
          {lessons.map((lesson: Lesson) => (
            <div key={lesson.id} className="border my-2 p-1">
              <div className="flex justify-center items-center">
                <CircleCheckBig
                  size={"20px"}
                  className={`mr-2 ${isLessonComplete(lesson) ? "text-orange-400" : "text-black"}`}
                />
                <button
              ...
              <div
                className={`task mt-2 space-y-2 overflow-hidden transition-all duration-500 ${expanded === lesson.id && lesson.tasks.length > 0 ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}
              >
                {lesson.tasks.map((task: Task) => (
                  ...
                  >
                    <CircleCheckBig
                      size={"20px"}
                      className={`mr-2 ${task.progress === 100 ? "text-orange-400" : "text-black"}`}
                    />
                    <span className="text-xs lg:text-sm flex-1">
                      {task.title}
                    </span>
                    <span className="text-xs lg:text-sm text-gray-600 mr-auto">
                      {task.progress}% Done
                    </span>
                  </Link>
                ))}
        ...
        <div className="notes col-span-4 grid grid-cols-2 lg:grid-cols-1 gap-2 lg:justify-items-end">
          <div className="static-note lg:h-[160px] lg:w-[285px] my-3 shadow-md shadow-gray-400 py-3 bg-[#AFE9D9]">
            <h3 className="note-title text-xs lg:text-md font-bold px-3">
              Important Note
            </h3>
            <hr className="my-1" />
            <p className="text-[9px] lg:text-sm px-3 lg:py-1">
              Content of week 3 will not be unlocked until week 2 is{" "}
              <span className="font-bold">80% completed.</span> Delays will
              cause automatic adjustments to the learning plan.
            </p>
          </div>
          <div className="danger-note lg:h-[160px] lg:w-[285px] my-3 shadow-md shadow-gray-400 py-3 bg-[#FFA7C4]">
            ...
            <hr className="my-1" />
            <p className="text-[9px] lg:text-sm px-3 lg:py-1">
              Week 2 ended without finishing the required materials. Do you want
              to extend the week or adjust your learning pace?
            </p>
            <div className="btns px-3 sm:flex lg:justify-between flex-wrap items-center mt-2">
              <MainLink
                title="Adjust Learning Pace"
                className="bg-black! p-0! lg:p-2! text-center lg:text-left text-[9px] text-white block! lg:inline-block! font-normal lg:font-bold"
                route="#"
              />
              <MainLink
                title="Extend"
